...
public class RacingTest {
    ...
    void When_RacingHasOneWinner_Return_RacingResult() {
        racing.start(Arrays.asList(3,5,4));
        Assertions.assertThat(racing.win().equals(new Cars(List.of(new Car("crong",5))))).isTrue();
    }
    ...
    void When_RacingHasTwoWinners_Return_RacingResultAsList() {
        ...
        Assertions.assertThat(racing.win().equals(new Cars(List.of(new Car("crong", 5),new Car("honux",5))))).isTrue();
    }

    @Test
    @DisplayName("생성된 자동차의 수와 움직인 수가 다르면 예외를 발생시킨다.")
    void When_CarsAndMovementCntDifferent_Throw_RuntimeException() {
        Assertions.assertThatThrownBy(() -> racing.start(Arrays.asList(3,5)))
                .isInstanceOf(RuntimeException.class)
                .hasMessageContaining("생성된 자동차의 수와 입력된 움직임의 수가 다릅니다.");
    }
    ...
    void When_NoCarsMove_Throw_RuntimException() {
        Assertions.assertThatThrownBy(() -> {
            ...
            racing.win();
        }).isInstanceOf(RuntimeException.class).hasMessageContaining("아직 게임이 시작되지 않았습니다.");
    }
